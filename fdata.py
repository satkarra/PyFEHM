...
class fzone(object):						#FEHM zone object.
    ...
    def _set_permeability(self,value): 
        ...
        if self.index in list(self._parent.perm.keys()):
            if self._updateFlag:
    ...
    def _set_conductivity(self,value): 
        ...
        if self.index in list(self._parent.cond.keys()):
            if self._updateFlag:
    ...
    def _set_property(self,value,prop0,props,macro):
        ...
        ks = list(self._parent.__getattribute__(macro).keys())
        if self.index in ks:
            if self._updateFlag:
    ...
    def _set_Pi(self,value): 
        ...
        if not self._parent: 
            pyfehm_print('Zone not associated with input file, no macro changes made.',self._silent)
            return
        if self.index in list(self._parent.pres.keys()):
            if self._updateFlag:
...
class fmacro(object): 						#FEHM macro object
    ...
    def _check_type(self):
        ...
        if self.type not in list(macro_list.keys()): return None
...
class fmodel(object): 						#FEHM model object.
    ...
    def _check_type(self):
        '''Determine if macro is supported.'''
        if self.type not in list(model_list.keys()): return None
    def _assign_param(self): 
        '''Assign parameters if supplied on initialisation.'''
        if self.index in list(model_list[self.type].keys()):
            self._param = dict([(par,None) for par in model_list[self.type][self.index]])
        else: 
            self._param = {}
    def _set_param2(self,param):
        '''Assign keys in param attribute appropriate to specific macro.'''
        if self.index not in list(model_list[self.type].keys()):	
            self._param = dict([('param'+str(i+1),par[1]) for i,par in enumerate(param)])	
            return																		
        elif param.__len__() != len(list(self._param.keys())): 
            return		# return if numbers don't match up
        if self.index in list(model_list[self.type].keys()):
            paramDict = model_list[self.type][self.index]
        else:
            paramDict = ['param'+str(i+1) for i in range(len(list(self._param.keys())))]
    ...
    def _check_zone(self):
        ...
        if self._parent:
            newlist = []
            for zn in self.zonelist:
                if zn in list(self._parent.zone.keys()):
...
class ftrac(object): 						#FEHM chemistry module.
    ...
    def add_common_model(self, zone = None, diffusion_model = 0, diffusion = 1.e-9, dispersion = [0.005,0.005,0.005]):
        ...
        if isinstance(cm.zone,int) or isinstance(cm.zone,str):
            if cm.zone in list(self._parent.zone.keys()): cm.zone = self._parent.zone[cm.zone]
...
class fgeneral(object): 					#PyFEHM general (undefined) macro class
    def __init__(self, lines, insert_after, zonelist, parent):
        ...
        if type(self.zonelist) in [int, str]:
            if self.zonelist in list(self._parent.zone.keys()):
...
class frlpm(object): 						#FEHM relative permeability object (different to rlp macro).
    ...
    def delete(self,model):
        ...
        if model in list(self._relperm.keys()): self._relperm.__delitem__(model)
        if model in list(self._capillary.keys()): self._capillary.__delitem__(model)
    ...
    def _get_phases(self):
        if len(self.relperm) == 0: return []
            ...
            return [self.relperm[k].phase for k in list(self.relperm.keys())]
...
class _relperm(object): 						# private class for relative permeability model
    ...
    def _set_param(self,param):
        if len(param) != len(list(self.param.keys())): return		# return if numbers don't match up
    ...
    def _set_type(self,value): 
        ...
        if value not in list(rlpm_dicts.keys()): _buildWarnings('WARNING: '+str(value)+' not an available model.');return
        self._type = value
...
class fdata(object):						#FEHM data file.
    ...
    __slots__=['_silent','_gridfilename','_inconfilename','_sticky_zones','_allMacro','_allModel','_associate',
            ...
            '_output_times','_path','_vtk','_storage','_air']
    def __init__(self,filename='',gridfilename='',inconfilename='',sticky_zones=dflt.sticky_zones,associate=dflt.associate,work_dir = None,
        ...
        self._allMacro = dict([(key,[]) for key in list(macro_list.keys())])
        self._allModel = dict([(key,[]) for key in list(model_list.keys())])
    ...
    def read(self,filename='',gridfilename='',inconfilename='',full_connectivity=dflt.full_connectivity,skip=[]):			#Reads data from file.
        ...
        infile = open(self._path.full_path,'r')
        read_fn=dict(list(zip(fdata_sections,
        ...
        while more:
            ...
            if keyword in fdata_sections and keyword not in skip:
                ...
                fn=read_fn[keyword]
                if keyword in list(macro_list.keys()):
                    self._read_macro(infile,keyword)
                    precedingZoneKey = None
                elif keyword in list(model_list.keys()):
    ...
    def _add_boundary_zones(self): 						#Automatically creates zones corresponding to x,y,z boundaries
        ...
        if self.grid.dimensions == 2:
            ...
            for k in ks:
                if k in list(self.zone.keys()): self.delete(self.zone[k])
    ...
    def _write_unparsed(self,outfile,key):
        ...
        if key in list(self._unparsed_blocks.keys()):
    ...
    def _add_boun(self,boun=fboun()):							#Adds a BOUN model.
        ...
        for zn in boun.zone:
            if isinstance(zn,tuple): 
                ...
                if zn in list(self.zone.keys()): zns.append(self.zone[zn])
            elif isinstance(zn,fzone):
                zns.append(zn)
        boun.zone = zns
    ...
    def print_ctrl(self):										#Prints out variables contained in CTRL.
        ...
        '''
        for k in list(self.ctrl.keys()):	print(k +': ' + str(self.ctrl[k]))
    ...
    def _read_flxo(self,infile): 							#FLXO: Reads FLXO macro.
        ...
        if nums[0].isdigit():
            ...
            for i1,i2 in zip(list(range(0,node_num,2)),list(range(0,node_num,2))):
    ...
    def print_iter(self):										#Prints out variables contained in ITER.
        ...
        '''
        for k in list(self.iter.keys()):	print(k +': ' + str(self.iter[k]))
    ...
    def _read_rlpm(self,infile):							#RLPM: Reads RLPM macro.		
        ...
        self._rlpmlist = list(rlpms.values())			
    ...
    def _write_ngas(self,outfile):								#Writes NGAS and associated macros.
        ...
        if self.sticky_zones:
            zns = []
            zns += list(self.ngas.init_pres.keys())
            zns += list(self.ngas.ncg_pres.keys())
            zns += list(self.ngas.source.keys())
            zns2 = []
            for zn in zns:
                if isinstance(zn,tuple): zns2.append(zn)
                elif isinstance(zn,(int,str)): 
                    if zn in list(self.zone.keys()):
        ...
        if 0 in list(self.ngas.init_pres.keys()):
            outfile.write('1\t0\t0\t'+str(self.ngas.init_pres[0])+'\n')
        for k in list(self.ngas.init_pres.keys()):
            if k == 0: continue
        ...
        if 0 in list(self.ngas.ncg_pres.keys()):
            outfile.write('1\t0\t0\t'+str(self.ngas.ncg_pres[0])+'\n')
        for k in list(self.ngas.ncg_pres.keys()):
            if k == 0: continue
            if isinstance(k,tuple):
        ...
        if 0 in list(self.ngas.source.keys()):
            outfile.write('1\t0\t0\t'+str(self.ngas.source[0])+'\n')
        for k in list(self.ngas.source.keys()):
            if k == 0: continue
            if isinstance(k,tuple):
    ...
    def print_time(self):										#Prints out variables contained in TIME.
        ...
        '''
        for k in list(self.time.keys()):	print(k +': ' + str(self.time[k]))
    ...
    def new_zone(self,index=None,name=None,rect=None,nodelist=None,file=None,from_file = None,permeability=None,conductivity=None,density=None,
        ...
        if index in list(self.zone.keys()):
            if overwrite:
        ...
        if permeability:
            ...
            # assign
            if index in list(self.perm.keys()):
        ...
        if conductivity:
            ...
            # assign
            if index in list(self.cond.keys()):
        ...
        if density or specific_heat or porosity:
            ...
            # assign
            if index in list(self.rock.keys()):
        ...
        if youngs_modulus or poissons_ratio:
            ...
            # assign
            if index in list(self.elastic.keys()):
        ...
        if thermal_expansion or pressure_coupling:
            ...
            # assign
            if index in list(self.biot.keys()):
        ...
        if Pi or Ti or Si:
            ...
            # assign
            if index in list(self.pres.keys()):
    ...
    def run(self,input='',grid = '',incon='',exe=dflt.fehm_path,files=dflt.files,verbose = None, until=None,autorestart=0,use_paths=False,no_paths = False,write_files_only = False, clean = False, writeSubFiles=True):
    ...
    def _write_prep(self):						#Determine if data object fit for writing
        ...
        ctrlFlag = dict_key_check(self.ctrl,list(dflt.ctrl.keys()),'ctrl')
        iterFlag = dict_key_check(self.iter,list(dflt.iter.keys()),'iter')
        timeFlag = dict_key_check(self.time,list(dflt.time.keys()),'time')
        solFlag = dict_key_check(self.sol,list(dflt.sol.keys()),'sol')
    ...
    def _read_zonn(self,infile,file=''):					#ZONE: Reads ZONE or ZONN macro.
        ...
        if line[0:4]=='file':
            ...
            while more:
                ...
                if zind in list(self.zone.keys()):
                ...
                if not zind in list(self.zone.keys()): self._add_zone(new_zone,overwrite=True)		
    ...
    def _add_zone(self,zone=fzone(),overwrite=False):			#Adds a ZONE object.
        ...
        if isinstance(zone,fzone):
            if zone.index in list(self.zone.keys()):
                if not overwrite:
            ...
            if zone.name in list(self.zone.keys()):
                if not overwrite:
            ...
            if self.permmodellist:
                ...
                for perm_model in self.permmodellist:
                    ...
                    for par,unit in zip(permDicts[perm_model.index],permUnits[perm_model.index]):
                        if par in list(perm_model.param.keys()):
    ...
    def _add_macro(self,macro,overwrite=False):					#Adds the macro to data file
        ...
        if isinstance(macro.zone,list) and len(macro.zone)==0: macro.zone = self.zone[0] 	# assign everywhere zone
            ...
            if macro.zone in list(self.zone.keys()): macro.zone = self.zone[macro.zone]
    ...
    def _macro_zone(self,nums):									#Assigns zone to macro dictionary
        ...
        if (nums[0] == '1' and nums[1] == '0' and nums[2] == '0') or (int(float(nums[0]))<0):
            ...
            if k in list(self.zone.keys()): return self.zone[k]
    ...
    def _read_model(self,infile,modelName): 				#MODEL: Reads general format models
        ...
        while line != '':									# perm model specification
            ...
            #^^^^^^^^^^^^^^^^^^^^^^^^^^^ end exception ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            if m.index in list(model_list[modelName].keys()):
            ...
            m.param = dict(list(zip(parList,parVector))) 	# make dictionary
            models.append(m)
            line = infile.readline().strip()		
        line = infile.readline().strip()
    ...
    def _read_model_plastic(self,infile): 		
        ...
        m.zonelist = [0]
        parVector = [float(num) for num in nums[1:]] 	# parameter values
        if m.index in list(model_list['plasticmodel'].keys()):
    ...
    def _read_model_ppor(self,infile):
        ...
        if m.index in list(model_list['ppor'].keys()):
    ...
    def _add_model(self,model):
        ...
        if isinstance(model.zonelist,list) and len(model.zonelist)==0: model.zonelist = [self.zone[0]] 	# assign everywhere zone
            ...
            if model.zonelist in list(self.zone.keys()): 
        ...
        for zn in model.zonelist:
            ...
            elif zn in list(self.zone.keys()): newlist.append(self.zone[zn])
            else: 
    ...
    def _write_model(self,outfile,modelName):
        ...
        for model in self._allModel[modelName]:
            ...
            for key in paramList:
                if key in list(model.param.keys()):
    ...
    def _write_model_ppor(self,outfile):
        ...
        for model in self._allModel['ppor']:
            if model.index in list(model_list['ppor'].keys()):
    ...
    def _write_model_plastic(self,outfile):
        ...
        for model in self._allModel['plasticmodel']:
            if model.index in list(model_list['plasticmodel'].keys()):
